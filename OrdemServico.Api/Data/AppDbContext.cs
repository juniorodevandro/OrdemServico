...
namespace OrdemServico.Data
{
    public class AppDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            {
                new() { Id = 1, Nome = "Próprio" },
                new() { Id = 2, Nome = "Terceiro" },
                new() { Id = 3, Nome = "Mão de obra" }
            });

            modelBuilder.Entity<TipoPessoa>().HasData(new List<TipoPessoa>
            {
                new() { Id = 1, Nome = "Física" },
                new() { Id = 2, Nome = "Jurídica" },
                new() { Id = 3, Nome = "Outro" }
            });

            modelBuilder.Entity<TipoOrdem>().HasData(new List<TipoOrdem>
            {
                new() { Id = 1, Nome = "Manutenção" },
                new() { Id = 2, Nome = "Compra" },
                new() { Id = 3, Nome = "Venda" }
            });            
            
            modelBuilder.Entity<Situacao>().HasData(new List<Situacao>
            {
                new() { Id = 1, Nome = "Cadastrado" },
                new() { Id = 2, Nome = "Ag modificação" },
                new() { Id = 3, Nome = "Ag aprovação" },
                new() { Id = 4, Nome = "Ativo" },
                new() { Id = 5, Nome = "Cancelado" },
            });
